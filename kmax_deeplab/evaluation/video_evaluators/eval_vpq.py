...
def VPQeval(predictions, pan_gt_json_file, pred_dir, gt_folder, output_dir):
    ...
    for video_images in pbar:
        ...
        for imgname_j in gt_image_jsons:
            ...
            pred_pans.append(os.path.join(pred_dir, video_id, imgname))
            gt_pans.append(os.path.join(gt_folder, video_id, imgname))
    ...
    for nframes in [1, 2, 4, 6]:
        ...
        vpq_all_, vpq_thing_, vpq_stuff_ = vpq_compute_parallel(
                gt_pred_split_, categories, nframes, output_dir, 8)
    ...
    output_file.close()
    print('==> VPQ time All:', time.time() - start_all, 'sec')


    res = {}
    res["VPQ all"] = sum(vpq_all)/len(vpq_all)
    res["VPQ thing"] = sum(vpq_thing)/len(vpq_thing)
    res["VPQ stuff"] = sum(vpq_stuff)/len(vpq_stuff)

    results = OrderedDict({"video_panoptic_seg": res})
    _print_video_panoptic_results(res)
