...
def _get_vip_seg_meta():
    ...
    thing_classes = [k["name"] for k in VIPSEG_CATEGORIES]
    thing_colors = [k["color"] for k in VIPSEG_CATEGORIES]
    stuff_classes = [k["name"] for k in VIPSEG_CATEGORIES]
    stuff_colors = [k["color"] for k in VIPSEG_CATEGORIES]
...
def register_vipseg_panoptic_annos_sem_seg(
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_vipseg_panoptic_json(panoptic_json, image_root, panoptic_root, metadata, is_test),
    )
    MetadataCatalog.get(panoptic_name).set(
...
# Video Loader
def load_vipseg_panoptic_json(json_file, video_dir, gt_dir, meta, is_test=False):
    """
    Args:
    ...
    if is_test:
        ...
        for video_ann in json_info["annotations"]:
            ...
            for idx, img_ann in enumerate(video_ann["annotations"]):
                ...
                label_file = os.path.join(gt_path, img_ann["file_name"])
