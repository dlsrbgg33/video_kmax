...
__all__ = ["BURSTVideoDatasetMapper"]
...
class BURSTVideoDatasetMapper:
        ...
        *,
        ...
        if is_train:
            logging.getLogger(__name__).info(
                "[BURSTVideoDatasetMapper] Full TransformGens used in training: {}, {}".format(
                    str(self.tfm_gens), str(self.tfm_gens_copy_paste)
                )
            )
        else:
            logging.getLogger(__name__).info(
                "[BURSTVideoDatasetMapper] Full TransformGens used in testing: {}".format(
                    str(self.tfm_gens)
                )
            )
        ...
        image_dir = "./datasets/burst/frames/train"
        json_file = "./datasets/burst/coco_annotations/train.json"
        gt_dir = "./datasets/burst/panopRGB_re"
        from ..datasets import burst
        meta_data = burst._get_burst_seg_meta()
        
        # load all videos
        self.dataset_dict_all = burst.load_burst_video_json(
            json_file, image_dir, gt_dir, meta_data, is_test
        )
        ...
        for idx, frame_idx in enumerate(selected_idx):
            ...
            if not self.is_train:
                ...
                if "pan_seg_file_name" in current_dataset_dict:
                    pan_seg_gt = utils.read_image(current_dataset_dict.pop("pan_seg_file_name"), "BGR")
    ...
    def call_copypaste(self, dataset_dict):
        ...
        # update the dataset_dict with keys needed during training or inference
        dataset_dict["image"] = []
        ...
        # the image that is copied is from another video.
        ...
        for i in range(len(dataset_dict_list)):
            ...
            if i == 0:
                for index, label_id in enumerate(all_ids):
                    # randomly copy labels.
                    # if index < keep_number
                    if index < keep_number:
                        copy_paste_masks[pan_seg_gt_copy_paste_list[i] % 256 == label_id] = 1
                        clip_copy_label_list.append(label_id)
            else:
                # if label_id is in clip_copy_label_list, then copy-paste.
            ...
            for segment_info in segments_info:
                ...
                if "iscrowd" not in segment_info:
                    segment_info["iscrowd"] = 0
                if not segment_info["iscrowd"]:
                    # -1 is reserved to indicate padded pixels.
                    if segment_info["id"] % 256 in [-1, 0]:
                        print(segment_info)
                        raise ValueError("id should not be -1, 0")
                    
                    binary_mask = (pan_seg_gt % 256 == segment_info["id"] % 256)
                    ...
                    if np.any(binary_mask):
                        ...
                        seg_id.append(segment_info["id"] % 256)
            ...
            for segment_info in segments_info_copy_paste:
                ...
                if not segment_info["iscrowd"]:
                    ...
                    if np.any(binary_mask):
                        ...
                        classes.append(class_id)
                        masks.append(binary_mask)
                        seg_id.append(segment_info["id"] % 256 + 256)
