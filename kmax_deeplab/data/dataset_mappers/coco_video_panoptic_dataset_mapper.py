...
__all__ = ["COCOVideoPanopticDatasetMapper"]
...
class COCOVideoPanopticDatasetMapper:
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            tfm_gens: data augmentationz
            tfm_gens_copy_paste: data augmentation
            tfm_color_jitter_gens: data augmentation for sequence-level color jitter
        ...
        image_dir_t = "./datasets/coco/train2017"
        gt_dir_t = "./datasets/coco/panoptic_train2017"
        semseg_dir_t = "./datasets/coco/panoptic_semseg_train2017"
        json_file_t = "./datasets/coco/annotations/panoptic_train2017.json"
        
        image_dir_v = "./datasets/coco/val2017"
        gt_dir_v = "./datasets/coco/panoptic_val2017"
        semseg_dir_v = "./datasets/coco/panoptic_semseg_val2017"
        json_file_v = "./datasets/coco/annotations/panoptic_val2017.json"

        from ..datasets import coco
        meta_data = coco._get_coco_meta()

        dataset_dict_t = coco.load_coco_panoptic_json(
            json_file_t, image_dir_t, gt_dir_t, semseg_dir_t, meta_data
        )

        dataset_dict_v = coco.load_coco_panoptic_json(
            json_file_v, image_dir_v, gt_dir_v, semseg_dir_v, meta_data
        )

        self.dataset_dict_all = dataset_dict_t + dataset_dict_v
    ...
    def read_dataset_dict(self, dataset_dict, is_copy_paste=False):
        ...
        # color jittering (autoaug) first for original image
        # Then, we copy the image for multiple frames
        if self.is_train:
            image, transforms_color = T.apply_transform_gens(self.tfm_color_jitter_gens, image)
        dataset_dict["image_color_jittered"] = image
        dataset_dict_copy = [dataset_dict] * self.sampling_frame_num
        
        datast_dict_list = []
        pan_seg_gt_list = []

        for frame_idx in range(len(dataset_dict_copy)):
            # make the list for multiple frames
            current_dataset_dict = copy.deepcopy(dataset_dict_copy[frame_idx])
            ...
            image = current_dataset_dict["image_color_jittered"]
            # generate pseudo video with applying different affine-transformation augmentation
            if not is_copy_paste:
                image, transforms = T.apply_transform_gens(self.tfm_gens, image)
            else:
                image, transforms = T.apply_transform_gens(self.tfm_gens_copy_paste, image)

            # image = _convert_to_rgb(np.ascontiguousarray(image.transpose(2, 0, 1)))
    ...
    def call_copypaste(self, dataset_dict):
        ...
        main_image_idx = self.filename2idx[dataset_dict["file_names"][0].split('/')[-1].replace('.jpg', '')]
        ...
        for i in range(len(dataset_dict_list)):
            ...
            # datset_mixed = self.openai_tranform(dataset_mixed)
