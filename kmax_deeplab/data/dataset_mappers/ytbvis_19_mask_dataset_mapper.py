...
class YTVISDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        image_dir = "./datasets/ytvis_2019/train/JPEGImages"
    ...
    def preprocess_dataset_dict(self, dataset_dict):
        ...
        for frame_idx in selected_idx:
            ...
            # new_h, new_w = image_shape[0], image_shape[1]
            # padded_image = np.zeros((self.image_size[0], self.image_size[1], 3), dtype=image.dtype)
            # padded_image[:new_h, :new_w] = image

            dataset_dict["image"].append(torch.as_tensor(np.ascontiguousarray(image.transpose(2, 0, 1))))
            # dataset_dict["image"].append(torch.as_tensor(np.ascontiguousarray(padded_image.transpose(2, 0, 1))))
    ...
    def __call__(self, dataset_dict):
        ...
        res = self.preprocess_dataset_dict(dataset_dict)

        # while "instances" in res and res["instances"][0].gt_ids.shape[0] == 0:
        if self.is_train:
            while "instances" in res and len(res["instances"][0].gt_ids) == 0:
