...
def get_2d_sincos_positoinal_embedding(embedding_dimension, height, width):
    ...
    return positoinal_embedding.reshape([1, embedding_dimension, height, width])
...
class kMaXPredictor(nn.Module):
    ...
    def forward(self, mask_embeddings, class_embeddings, pixel_feature, frame2=False):
        ...
        if frame2:
            ...
            pixel_space_normalized_feature = pixel_space_normalized_feature.reshape(-1, 1, C, H, W)
        ...
        # mask_logits = mask_logits[0].unsqueeze(0)
...
class kMaXTransformerLayer(nn.Module):
    ...
    def forward(self, pixel_feature, query_feature):
        ...
        # pixel_space_input = pixel_feature.reshape(N, C, -1) # N//num_frames, C, H*W*num_frames
        
        # pixel_space_input = self.with_pos_embed(pixel_space_input, pos.permute(1,2,0))

        # pixel_space = self._pixel_conv1_bn_act(F.gelu(pixel_space_input)) # N//num_frames, C, H*W*num_frames
        ...
        # pixel_value_list = torch.split(pixel_value, 1, dim=0)
        # pixel_value = pixel_value.reshape(N, self._total_value_depth, H*W*self.num_frames)
        pixel_value = pixel_value.reshape(-1, self._total_value_depth, H*W)
        
        pixel_value = pixel_value[0].unsqueeze(0)
        pixel_value = torch.cat([pixel_value, pixel_value], dim=-1)
        # k-means assignment.

        # pixel_space_2d = pixel_space.reshape(-1, self._bottleneck_channels, H, W)[0].unsqueeze(0)
        pixel_space_2d = pixel_space.reshape(-1, self._bottleneck_channels, H, W)
        # pixel_space_2d = torch.cat([pixel_space_2d, pixel_space_2d], dim=0)
        prediction_result = self._predcitor(
            mask_embeddings=query_space, class_embeddings=query_space, pixel_feature=pixel_space_2d, frame2=True)
        # clustering_result = prediction_result['mask_logits'].flatten(2).detach() # N L HW
        # clustering_result = prediction_result['mask_logits'][0].unsqueeze(0) # N L HW
        clustering_result = prediction_result['mask_logits'] # N L HW
        # import pdb; pdb.set_trace()

        with torch.no_grad():
            # clustering_result = prediction_result['mask_logits'].flatten(2).detach() # N L HW
            # clustering_result = prediction_result['mask_logits'].permute(1,0,2,3).unsqueeze(0).flatten(2).detach()
            # clustering_result = clustering_result.reshape(N, clustering_result.shape[1], -1).detach()
        ...
        # k-means update.
        _, _, P = clustering_result.shape
        # clustering_result_list = torch.split(clustering_result, P // 2, dim=-1)
        # kmeans_update_list = []
        # for i in range(self.num_frames):
        #     pixel_value = pixel_value_list[i].reshape(N, self._total_value_depth, H*W)
        #     kmeans_update_list.append(torch.einsum('blm,bdm->bdl', clustering_result_list[i].float(), pixel_value.float()))
        # kmeans_update = torch.mean(torch.stack(kmeans_update_list, dim=0), dim=0)
        # clustering_result
        # clustering_result = clustering_result[:, :, :P//2]
        # pixel_value = pixel_value[:, :, :P//2]
        # pixel_value = pixel_value[:, :, :P]
        # clustering_result = torch.cat([clustering_result, clustering_result], dim=-1)
...
class kMaXTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.pe_layer_list = []
        for i in range(4):
            self.pe_layer_list.append(PositionEmbeddingSine3D(N_steps, normalize=True))
            N_steps = N_steps // 2
    ...
    def forward(self, x, panoptic_features, semantic_features):
        ...
        # bt, c_m, h_m, w_m = panoptic_features.shape
        ...
        #     pos.append(self.pe_layer_list[i](x[i].view(bs, t, -1, size_list[-1][0], size_list[-1][1]), None).flatten(3))
            
        #     # NTxCxHW => NxTxCxHW => (TxHW)xNxC
        #     _, _, c, hw = pos[-1].shape
        ...
        for i, feat in enumerate(x):
            for _ in range(self._num_blocks[i]):
                ...
                # emb, height, width = feat.shape[1:]
                # positional_emb = get_2d_sincos_positoinal_embedding(emb, height, width)
                # positional_emb = torch.from_numpy(positional_emb).to(feat.device)
                # import pdb; pdb.set_trace()
                # feat += positional_emb
                # cluster_centers, prediction_result = self._kmax_transformer_layers[current_transformer_idx](
                #     pixel_feature=feat, query_feature=cluster_centers, pos=pos[i]
                # )
                cluster_centers, prediction_result = self._kmax_transformer_layers[current_transformer_idx](
