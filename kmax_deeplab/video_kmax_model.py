...
# loading sam related modules
from kmax_deeplab.sam_repo.segment_anything import SamAutomaticMaskGenerator, sam_model_registry, SamPredictor
import open_clip
...
class VideokMaX(kMaXDeepLab):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Args:
            mostly inherit from kMaXDeepLab
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if self.debug_only:
                # only if you want to visualize the training input
            ...
            features = self.backbone(images_padded.tensor)
            ...
            num_test_samples = images_padded.tensor.shape[0]
            processed_results = []
            # define "test_video_tensor" with temporal padding
            last_image_tensor = images_padded.tensor[-1].expand(self.test_num_frames, -1, -1, -1)
            test_video_tensor = torch.cat((images_padded.tensor, last_image_tensor), dim=0)
            # define "test_video" (to recover padded images with original size) with temporal padding
            test_video = torch.stack(batched_inputs[0]["image"], dim=0).to(self.device)
            last_image = test_video[-1].expand(self.test_num_frames, -1, -1, -1)
            test_video = torch.cat((test_video, last_image), dim=0)
            ...
            stuff_video_memory = {}
            for i in range(0, images.tensor.shape[0], load_clip_gap):
                test_clip = test_video[i:i+self.test_num_frames]
                test_clip_tensor = test_video_tensor[i:i+self.test_num_frames]
                ...
                for mask_cls_result, mask_pred_result, input_per_image, image_size, cluster_result in zip(
                    mask_cls_results, mask_pred_results, batched_inputs, images.image_sizes, cluster_results
                ): 
                    ...
                    cur_image = test_clip[:, :, :image_size[0], :image_size[1]]
                    ...
                    if self.panoptic_on:
                        panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, height_wised_results, cluster_result, height)
                        # panoptic_r includes "panoptic_seg", "segments_info".
                        # "segments_info" is the dict with ['id', 'category_id', 'isthing', 'score']
                        # combine the panoptic_seg and segments_info into one [HxW], each pixel is label_divisor * category_id + id
                        ...
                        # video stitching 
                        if self.use_clip_stitching:
                            stitched_panoptic_seg, non_match_dict, matched_cur_to_prev, asso_dict_feat = self.clip_stitcher(
                            ...
                            if self.memory_name is not None:
                                ...
                                self.lamb.remove_stuff_from_matched(non_match_dict)
                                non_stitched_ids_feat = self.clip_stitcher.update_aux_ids(asso_dict_feat, non_match_dict)

                                self.lamb.remove_stuff_from_matched(matched_cur_to_prev)
                                stitched_ids_feat = self.clip_stitcher.update_aux_ids(asso_dict_feat, matched_cur_to_prev)
            ...
            processed_results = processed_results[:(images.tensor.shape[0])]
    ...
    def stuff_id_merge(self, panoptic_seg, stuff_video_memory):
        ...
        class_label = panoptic_seg // self.label_divisor
        class_label_unique = class_label.unique()

        if len(stuff_video_memory) > 0:
            for id in stuff_video_memory.keys():
                panoptic_seg_class = panoptic_seg // self.label_divisor
                if id in panoptic_seg_class:
                    panoptic_seg[panoptic_seg_class == id] = stuff_video_memory[id]
        ...
        for label in class_label_unique:
            if label not in self.metadata.thing_dataset_id_to_contiguous_id.values():
                ...
                panoptic_id = panoptic_seg[mask][0]
    ...
    def panoptic_inference(self, mask_cls, mask_pred, cluster_results, height):
        # mask_cls: N x C
