...
class VideoStitching(nn.Module):
    def __init__(self, cfg, num_frames, metadata):
        ...
        self._apply_intra_clip_asso = cfg.MODEL.VIDEO_KMAX.TEST.APPLY_INTRA_CLIP_ASSO
    ...
    def _intra_clip_associate(self, panoptic_seg, aux_ids_feat, seq_idx, height):
        ...
        # extract only ids that belong to "thing"
        ...
        location_scores = torch.exp(-location_gap/1.0)
        ...
        for idx in range(len(row_ind)):
            first_corres_id = first_not_overlap_thing_ids[row_ind[idx]]
            remain_corres_id = reamin_not_overlap_thing_ids[col_ind[idx]]
            score = total_score[row_ind[idx], col_ind[idx]]
            first_class, remain_class = (
                first_corres_id // self.label_divisor, remain_corres_id // self.label_divisor)

            if float(score) > self.score_thres and first_class == remain_class:
                binary_mask = (panoptic_seg == remain_corres_id)
                panoptic_seg[binary_mask] = first_corres_id
                aux_ids_feat[first_corres_id]["cluster_feat"] = (
                    aux_ids_feat[first_corres_id]["cluster_feat"] + aux_ids_feat[remain_corres_id]["cluster_feat"]) / 2.0
                aux_ids_feat[first_corres_id]["bbox_xyxy"] = (
                    aux_ids_feat[first_corres_id]["bbox_xyxy"] + aux_ids_feat[remain_corres_id]["bbox_xyxy"]) / 2.0
                del aux_ids_feat[remain_corres_id]
        return panoptic_seg, aux_ids_feat
    ...
    def forward(self, panoptic_seg, aux_ids_feat, seq_idx, height):
        ...
        if self._num_stitching_overlap_frames != 0 and self._apply_intra_clip_asso:
            panoptic_seg, aux_ids_feat = self._intra_clip_associate(
                panoptic_seg, aux_ids_feat, seq_idx, height)

        stitched_panoptic, non_match_dict, matched_cur_to_prev = (
            self._get_and_update_stitched_video_predictions(panoptic_seg, seq_idx))
    ...
    def _get_and_update_stitched_video_predictions(self, panoptic_seg, seq_idx):
        ...
        if seq_idx == 0:
            ...
            # The segment matching is conducted along the tracklet tubes.
    ...
    def _get_previous_clip_prediction_overlap(self, pred_panoptic, seq_idx):
        ...
        def _store_overlap_frame_prediction():
        ...
        prev_panoptic_overlap = (_store_overlap_frame_prediction() if seq_idx == 0
    ...
    def save_stitched_panoptic_to_buffer(self, stitched_panoptic):
        ...
        self.next_tracking_id = max(max_stitched_panoptic, self.next_tracking_id)
    ...
    def update_aux_ids(self, aux_dict, non_match_dict, i):
