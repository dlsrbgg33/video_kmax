        ...
        *,
        ...
        if is_train:
            logging.getLogger(__name__).info(
                "[VIPSegPanopticDatasetMapper] Full TransformGens used in training: {}, {}".format(
                    str(self.tfm_gens), str(self.tfm_gens_copy_paste)
                )
            )
        else:
            logging.getLogger(__name__).info(
                "[VIPSegPanopticDatasetMapper] Full TransformGens used in testing: {}".format(
                    str(self.tfm_gens)
                )
            )
        ...
        image_dir = "./datasets/vip_seg/train_images"
        gt_dir = "./datasets/vip_seg/panomasksRGB"
        json_file = "./datasets/vip_seg/panoptic_gt_VIPSeg_train.json"
        from ..datasets import vip_seg
        meta_data = vip_seg._get_vip_seg_meta()
        
        # load all videos
        self.dataset_dict_all = vip_seg.load_vipseg_panoptic_json(
